...
class ArticleMediaRepository(BaseRepository):
    async def get_all(self, page_params: PageParams = None):
        """
            Barcha ma'lumotlarni olish
        """
        query = select(ArticleMedia)
    ...
    async def get_one(self, id: int):
        ...
        """
        query = select(ArticleMedia).where(ArticleMedia.id==id)
        result = await self.session.execute(query)
    ...
    async def create(self, payload: dict, flush: bool = False, commit: bool = True) -> int | bool:
        ...
        """
        query = insert(ArticleMedia).values(payload)
    ...
    async def update(self, id: int, payload: dict, flush: bool = False, commit: bool = True) -> int | bool:
        ...
        """
        query = update(ArticleMedia).where(ArticleMedia.id==id).values(payload)
        exc = await self.session.execute(query)
    ...
    async def delete(self, id: int):
        ...
        """
        query = delete(ArticleMedia).where(ArticleMedia.id==id)
